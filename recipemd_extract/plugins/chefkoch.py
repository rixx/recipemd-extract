...
def extract(url,soup):
	...
	# servings
	servings= soup.find('input', attrs={'name':'portionen'}).attrs['value']
	yields=[Amount(Decimal(servings), f'Portion{"en" if int(servings) > 1 else ""}')]

	# tags
	tags=[]
	tagcloud=soup.find('amp-carousel', attrs={'class': 'ds-tags-carousel'})
	for tag in tagcloud.find_all('a'):
		tags.append(tag.text.strip(" \n"))
	# ingredients
	table = soup.find('table', attrs={'class': 'ingredients'})
	...
	# instructions
	instruct = soup.select('small.ds-recipe-meta + div')[0].text  # only get text
