...
from recipemd.data import Amount, Ingredient, Recipe
...
def extract(url, soup):
    ...
    summaryPars = soup.find(
        "div", attrs={"class": "recipe-introduction-body"}
    ).find_all("p")
    ...
    for step in soup.find_all("li", attrs={"class": "recipe-procedure"}):
        stepNumber = step.find(
            "div", attrs={"class": "recipe-procedure-number"}
        ).text.strip()
        stepInstr = step.find(
            "div", attrs={"class": "recipe-procedure-text"}
        ).text.strip()
